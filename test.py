...
def test(cfg,
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            #print('pred[:,6]: ',pred[:, 6].cpu())
            print('len(pred_cls): ',len(pred[:,6].cpu()))
            stats.append((correct, pred[:, 4].cpu(), pred[:, 6].cpu(), tcls))

    # Compute statistics
    stats = [np.concatenate(x, 0) for x in list(zip(*stats))]  # to numpy
    if len(stats):
        #print('ap_per_class(*stats): ',ap_per_class(*stats))
        #print('stats: ',stats)
        p, r, ap, f1, ap_class = ap_per_class(*stats)
       # print('p: ',p.shape,' r: ',r.shape,' ap: ',ap.shape,' f1: ',f1.shape)
    ...
    #print('stats[3]: ',stats[3])
    #print('stat[0].shape: ',stats[0].shape)
    #print('stat[2].shape: ',stats[2].shape)
    #print('stat[3].shape: ',stats[3].shape)
    #print('stats[3].shape: ',stats[3].shape)
    # Print results
